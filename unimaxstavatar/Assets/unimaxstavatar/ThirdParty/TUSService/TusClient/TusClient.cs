...
namespace TusDotNetClient
{
    ...
    public class TusClient
    {
            ...
            params (string key, string value)[] metadata)
        {
            ...
            var asset_resource_uuid = "";

            if (is_catalog)
            {
                metadata = metadata.Concat(new[] { ("is_catalog", "true") }).ToArray();
            }
            else
            {
                if (!asset_resource_uuid.IsNullOrEmpty())
                    metadata = metadata.Concat(new[] { ("asset_resource_uuid", asset_resource_uuid) }).ToArray();
            }
            ...
            params (string key, string value)[] metadata)
        {
            ...
            UnityEngine.Debug.Log($"[TusClient] CreateAsync requestUri : {requestUri}");
            ...
            {
                UnityEngine.Debug.Log("[TusClient] CreateAsync request.Headers : Key: " + entry.Key + ", Value: " + entry.Value);
            }

            var response = await client.PerformRequestAsync(request)
                .ConfigureAwait(false);

            UnityEngine.Debug.Log($"[TusClient] CreateAsync response : {response}");
            ...
            var location = response.Headers["Location"] ?? throw new Exception("Location Header Value Null Exception");

            UnityEngine.Debug.Log($"[TusClient] CreateAsync location : {location}");

            if (!Uri.TryCreate(location, UriKind.RelativeOrAbsolute, out var locationUri))
                throw new Exception("Invalid Location Header");

            if (!locationUri.IsAbsoluteUri)
                locationUri = new Uri(requestUri, locationUri);

            UnityEngine.Debug.Log($"[TusClient] CreateAsync locationUri.AbsolutePath : {locationUri.AbsolutePath}");

            return locationUri.AbsolutePath;
        }
            ...
            return new TusOperation<List<TusHttpResponse>>(
                async reportProgress =>
                {
                    try
                    {
                        ...
                        UnityEngine.Debug.Log($"[TusClient] UploadAsync : Uploading {offset}");
                        ...
                        UnityEngine.Debug.Log($"[TusClient] UploadAsync : uploadChunkSize {uploadChunkSize}");
                        ...
                        while (offset < fileStream.Length)
                        {
                            ...
                            foreach (var entry in request.Headers)
                            {
                                UnityEngine.Debug.Log("[TusClient] UploadAsync request.Headers : Key: " + entry.Key + ", Value: " + entry.Value);
                            }
                            ...
                            try
                            {
                                ...
                                UnityEngine.Debug.Log($"[TusClient] UploadAsync : response {response}");
                            ...
                            {
                                UnityEngine.Debug.Log($"[TusClient] UploadAsync : catch IOException ex {ex}");
        ...
        {
            UnityEngine.Debug.Log($"[TusClient] GetFileOffset url : {url}");
            ...
            UnityEngine.Debug.Log($"[TusClient] response  : {response}");
