...
Shader "TextMeshPro/Distance Field-BillboardFacing Orthographic"
{
    ...
    Properties{
		...
		_Orthographic_Size("Orthographic_Size", Range(0.1, 9999)) = 1
    }
        ...
        Pass {
            ...
            pixel_t VertShader(vertex_t input)
            {
				...
				float3 objScale = float3(
					length(unity_ObjectToWorld._m00_m10_m20) * _Orthographic_Size,
					length(unity_ObjectToWorld._m01_m11_m21) * _Orthographic_Size,
					length(unity_ObjectToWorld._m02_m12_m22) * _Orthographic_Size);

				float4 vPosition = mul(UNITY_MATRIX_P,
					mul(UNITY_MATRIX_MV, float4(0.0, 0.0, 0.0, 1.0))
					+ float4(vert.x, vert.y, 0.0, 0.0)
					* float4(objScale.x, objScale.y, objScale.z, 1.0));

                float2 pixelSize = vPosition.w;
                pixelSize /= float2(_ScaleX, _ScaleY) * abs(mul((float2x2)UNITY_MATRIX_P, _ScreenParams.xy));
                float scale = rsqrt(dot(pixelSize, pixelSize));
                scale *= abs(input.texcoord1.y) * _GradientScale * _Orthographic_Size * 2;
